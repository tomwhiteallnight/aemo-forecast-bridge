...
def fetch_aemo_data(report_type):
    ...
    # FILTER LOGIC:
    # For actuals, we want the "2-day rolling" file, not the latest 5-min interval.
    # These usually look like 'PUBLIC_DISPATCHPRICE_...' 
    if report_type == "actuals":
        # Find files that look like the rolling window dispatch price report
        target_files = [f for f in all_files if "DISPATCHPRICE" in f.upper()]
        selected_file = sorted(target_files)[-1] # Gets the latest rolling window file
    else:
        selected_file = sorted(all_files)[-1]
    ...
    with zipfile.ZipFile(io.BytesIO(zip_response.content)) as z:
        # We need to find the CSV inside the zip. Some zips have multiple.
