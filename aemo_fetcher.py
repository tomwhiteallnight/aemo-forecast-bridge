...
def fetch_aemo_data(report_type):
    # For actuals, we look in the DispatchIS_Reports folder but specifically for PRICE files
    ...
    if report_type == "actuals":
        # CRITICAL CHANGE: Look for DISPATCHPRICE, not DISPATCHIS
        # These files cover the rolling window of the last 48 hours
        target_files = [f for f in all_files if "PUBLIC_DISPATCHPRICE" in f.upper()]
    else:
        target_files = [f for f in all_files if "PUBLIC_PREDISPATCHIS" in f.upper()]
        
    if not target_files:
        print(f"Warning: No files found for {report_type}")
        return []
            
    # Sort and pick the largest file (often the one containing the full window)
